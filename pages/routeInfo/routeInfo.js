...
Page({
  ...
  onLoad: function (options) {
    ...
    this.setData({
      stopName: stopName, stopOption: stopOption, endIndex: stopName.length - 3, stations: stopName
    });
    ...
    this.rect = {//圆角矩形对象
      ...
      img_r: '/images/rect-r.png',//终点图片
      img_n: '/images/rect-n.png',
      height: 32,
      width: 68,

      //注：前端读取这两个点
      start_idx: this.data.startIndex,//传进来的起点
      end_idx: this.data.endIndex,
    }
  ...
  initRect: function (idx) {
    //我的修改
    if (idx == this.rect.end_idx) {
      const rectArr = this.rectArr;
      let x = rectArr[idx].x;
      let y = rectArr[idx].y;
      this.ctx.drawImage(this.rect.img_r, x, y, this.rect.width, this.rect.height);
    } else if (idx > this.rect.end_idx){
      const rectArr = this.rectArr;
      let x = rectArr[idx].x;
      let y = rectArr[idx].y;
      this.ctx.drawImage(this.rect.img_n, x, y, this.rect.width, this.rect.height);
      }else {
      const rectArr = this.rectArr;
      let x = rectArr[idx].x;
      let y = rectArr[idx].y;
    ...
    if (touch.length == 1) {
      ...
      for (let i = 0; i < rectArr.length; i++) {
        ...
        if (i >= this.rect.end_idx) {
          ...
          if (inside) {
            this.rect.start_idx = i;
            this.chooseStation(i);
            this.setData({ startIndex: i })
            break;
          }
        }
      }
